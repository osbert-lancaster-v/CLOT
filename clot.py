...
def setRanks_ByNumWinsOnly(tourney_id):
	...
	#logging.info(finishedGamesGroupedByWinner)
	for game in games.Game.all().filter("tourney_id =", tourney_id): ###.run(batch_size=1000):
		#logging.info('game:')
		#logging.info(game)
	...
	playersMappedToNumWins = [(p, winCounts.get(p.key().id(), 0)) for p in players.Player.all().filter("tourney_id =", tourney_id)] ###.run(batch_size=1000)] 
...
def getFinishedGames(tourney_id):
	return games.Game.all().filter("winner !=", None).filter("tourney_id =", tourney_id)#.run(batch_size=1000)


def getPlayersIDNameDict(tourney_id):
	return dict([[p.key().id() , p] for p in players.Player.all().filter("tourney_id =", tourney_id)]) ###.run(batch_size=1000)])
